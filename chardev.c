...
// for module init and exit macros 
#include <linux/miscdevice.h> 
#include <linux/uaccess.h>          
// for copy_to_user, see code 
#include <asm/io.h> 
...
static ssize_t device_read(struct file *filp, char *buffer,  size_t length, loff_t *offset) {
    ...
    // if all message has been read, return 0 to mark EOF 
    if(*offset != 0){ 
        *offset = 0; 
        return 0; 
    } 
 
    // buffer must be big enough to read entire message at once 
    bytes = strlen (chardev_msg); 
    if(length < bytes){ 
        printk (KERN_ERR "Error: buffer's length too small\n"); 
        return -EMSGSIZE; 
    } 
 
    if (copy_to_user (buffer, chardev_msg, bytes) != 0){ 
